...
/** Reports which users are speaking, the speakers' volumes, and whether the local user is speaking.
 ...
 The SDK triggers two independent [reportAudioVolumeIndicationOfSpeakers]([AgoraRtcEngineDelegate rtcEngine:reportAudioVolumeIndicationOfSpeakers:totalVolume:]) callbacks at one time, which separately report the volume information of the local user and all the remote speakers. For more information, see the detailed parameter descriptions.

**Note:**

 - To enable the voice activity detection of the local user, ensure that you set `report_vad(YES)` in the [enableAudioVolumeIndication]([AgoraRtcEngineKit enableAudioVolumeIndication:smooth:report_vad:]) method.
...
/** Occurs when a remote user's video stream playback pauses/resumes.
 ...
 You can also use the [remoteVideoStateChangedOfUid]([AgoraRtcEngineDelegate rtcEngine:remoteVideoStateChangedOfUid:state:reason:elapsed:]) callback with the following parameters: 
...
/** Initializes the AgoraRtcEngineKit object.
  ...
  @warning Only users with the same App ID can call each other.
  @warning One AgoraRtcEngineKit can use only one App ID. If you need to change the App ID, call [destroy](destroy) to release the current instance first, and then call this method to create a new instance.
  @param config    App ID and Area Code are the members of the AgoraRtcEngineConfig.
    App ID issued to you by Agora. Apply for a new one from Agora if it is missing in your kit. Each project is assigned a unique App ID. The App ID identifies your project and organization in the [joinChannelByToken]([AgoraRtcEngineKit joinChannelByToken:channelId:info:uid:joinSuccess:]) method to access the Agora Global Network, and enable one-to-one or one-to-more communication or live-broadcast sessions using a unique channel name for your App ID.
    Area Code can be set:
    AgoraIpAreaCode_CN --> access China server domain
    AgoraIpAreaCode_NA --> access North American server domain
    AgoraIpAreaCode_EUR --> access Eurpon server domain
    AgoraIpAreaCode_AS --> access Asia server domain
    AgoraIpAreaCode_GLOBAL --> access gloabl server domain
 ...
 * In scenarios requiring high-quality audio, we recommend setting `profile` as AgoraAudioProfileMusicHighQuality(4) and `scenario` as AgoraAudioScenarioGameStreaming(3). For example, for music education scenarios.
 ...
 @param scenario Sets the audio application scenario. See [AgoraAudioScenario](AgoraAudioScenario). Under different audio scenarios, the device uses different volume tracks, i.e. either the in-call volume or the media volume. For details, see [What is the difference between the in-call volume and the media volume?](https://docs.agora.io/en/faq/system_volume).
...
- This method is different from the [muteLocalAudioStream]([AgoraRtcEngineKit muteLocalAudioStream:]) method:
  ...
  - [enableLocalAudio]([AgoraRtcEngineKit enableLocalAudio:]): Disables/Re-enables the local audio capturing and processing. If you disable or re-enable local audio recording using the `enableLocalAudio` method, the local user may hear a pause in the remote audio playback.
 ...
 **Note:**
 ...
 - If you call [setChannelProfile]([AgoraRtcEngineKit setChannelProfile:]) after this method, the SDK resets whether or not to mute the local audio according to the channel profile and user role. Therefore, we recommend calling this method after the `setChannelProfile` method.
...
/** Disables the local video.
 ...
 After the local video capturer is successfully disabled or re-enabled, the SDK triggers the [didLocalVideoEnabled]([AgoraRtcEngineDelegate rtcEngine:didLocalVideoEnabled:byUid:]) callback on the remote client.
 ...
 **Note:**
 ...
 - If you call [setChannelProfile]([AgoraRtcEngineKit setChannelProfile:]) after this method, the SDK resets whether or not to mute the local video according to the channel profile and user role. Therefore, we recommend calling this method after the `setChannelProfile` method.
...
See [Raw Audio Data](https://docs.agora.io/en/Interactive%20Broadcast/raw_data_audio_apple?platform=iOS).
 ...
 **Note**
 ...
 - If you only want to add a watermark image to the local video for the audience in the CDN live broadcast channel to see and capture, you can call this method or the [setLiveTranscoding]([AgoraRtcEngineKit setLiveTranscoding:]) method.
 ...
 - If you have mirrored the local video by calling the [setupLocalVideo]([AgoraRtcEngineKit setupLocalVideo:]) or [setLocalRenderMode]([AgoraRtcEngineKit setLocalRenderMode:mirrorMode:]) method, the watermark image in preview is also mirrored.
 ...
 **DEPRECATED** from v3.0.0, use the [setupLocalVideo]([AgoraRtcEngineKit setupLocalVideo:]) method instead.
 ...
 **DEPRECATED** from v3.0.0, use the [setupRemoteVideo]([AgoraRtcEngineKit setupRemoteVideo:]) method instead.
